...
Pins live under **`tektonMirror`** in **`hub-clusters/day2/hub-env-values/<env>/<site>/<hub>/values.yaml`**.
...
- **`gitSync`** — defaults for **`gitops-repo-url`**, **`hub-values-relative-path`**, **`github-repo-slug`**, branch names, commit user, PR title, policy manifest path, and **`skip-git-sync`** / **`skip-policy-manifest-sync`**.
- **`pipeline.mirrorImage`** — single **RHEL-based** runner (from **`custom-container-images/oc-mirror`**) used for **`oc mirror`**, artifact listing, and the Git **`yq`** / **`jq`** / **`gh`** promote step; no separate promote image.
