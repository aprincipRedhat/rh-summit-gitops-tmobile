...
**Pipeline charts.** Besides ZTP and mirror, we’re carrying bulk-ZTP and node-replacement pipelines under `cluster-automation/spoke-automation/`—each has its own `values.yaml` and README; fold new spokes into `99-pipeline-values` the same way as the existing examples.
